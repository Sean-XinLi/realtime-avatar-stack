...
export class PeerSignalClient {
  ...
  async #applyRemoteCandidate(candidate) {
    const keyCandidate = candidate?.candidate;
    if (!keyCandidate || !shouldUseIceCandidate(keyCandidate, this.iceConfig)) {
      return;
    }
    if (!this.remoteDescriptionReady) {
  ...
  async #pollRemoteCandidates() {
    ...
    while (!this.closed && !complete) {
      ...
      await new Promise((resolve) => {
        window.setTimeout(resolve, REMOTE_CANDIDATE_POLL_MS);
      });
    }
  }
