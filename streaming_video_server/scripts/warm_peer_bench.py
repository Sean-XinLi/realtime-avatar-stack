...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--session-hold-ms", type=int, default=1200)
    parser.add_argument("--session-gap-ms", type=int, default=300)
...
async def run_once(args: argparse.Namespace, run_index: int) -> dict[str, object]:
    user_data_dir = tempfile.mkdtemp(prefix="chrome-warm-bench-", dir="/tmp")
            ...
            + urllib.parse.urlencode(
                {
                    ...
                    "sessionHoldMs": args.session_hold_ms,
                    "sessionGapMs": args.session_gap_ms,
                    "run": run_index,
                }
            )
        )
